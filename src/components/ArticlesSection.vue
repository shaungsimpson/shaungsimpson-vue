    ...
    <div class="flex flex-col gap-16">
        ...
        <article v-for="article in articles" class="relative flex flex-col items-start group">
            <h2 class="text-base font-semibold tracking-tight text-strong">
                ...
                <router-link :to="article.link"><span class="absolute z-20 -inset-x-4 -inset-y-6 sm:-inset-x-6 sm:rounded-2xl">
                    </span>
                    <span class="relative z-10">{{ article.title }}</span>
                </router-link>
            </h2>
            <time class="relative z-10 order-first mb-3 flex items-center text-sm text-subtle pl-3.5"
            ...
            <p class="relative z-10 mt-2 text-sm text-default">{{ article.blurb }}</p>
